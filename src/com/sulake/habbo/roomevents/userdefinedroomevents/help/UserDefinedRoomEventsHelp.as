package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1931:IWindowContainer;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1931 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1248 = this.var_1931.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1248)
         {
            this.var_1248.procedure = this.onButtonClick;
         }
         this.var_1247 = this.var_1931.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1247)
         {
            this.var_1247.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_275 >= this.var_80.length - 1)
         {
            this.var_1247.disable();
         }
         else
         {
            this.var_1247.enable();
         }
         if(this.var_275 <= 0)
         {
            this.var_1248.disable();
         }
         else
         {
            this.var_1248.enable();
         }
      }
      ...
      {
         ++this.var_275;
         this.var_275 = Math.min(this.var_275,this.var_80.length - 1);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_83.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_80.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_80 || this.var_80.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_80[this.var_275][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_80[this.var_275][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_80[this.var_275][1]).value;
