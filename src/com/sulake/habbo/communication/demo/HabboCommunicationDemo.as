package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_540:IKeyExchange;
      
      private var var_993:String;
      
      private var var_17:HabboLoginDemoView;
      
      private var var_2686:PseudoRandom;
      ...
      private const const_1749:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_654 = param1;
            this.var_58.initConnection(HabboConnectionType.const_243);
         }
      }
      ...
      {
         if(this.var_58 && this.var_173 && this.var_15 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         this.var_233 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         var _loc1_:String = this.var_173.getKey("client.hotel_view.image");
         var _loc2_:String = this.var_173.getKey("image.library.url");
         if(_loc1_ != null && _loc2_ != null)
         {
            this.var_233.loadHotelViewImage(_loc2_ + _loc1_);
         }
         var _loc3_:* = this.var_173.getKey("use.sso.ticket") == "1";
         this.var_654 = this.var_173.getKey("sso.ticket",null);
         this.var_1900 = this.var_173.getKey("flash.client.url","");
         this.var_2684 = this.var_173.getKey("external.variables.txt","");
         if(this.var_233 == null)
         {
            this.var_233 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         }
         if(_loc3_)
         {
            this.var_58.mode = HabboConnectionType.const_92;
            if(this.var_654)
            {
               this.var_58.initConnection(HabboConnectionType.const_243);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc4_)
            {
               ...
               this.var_17.addEventListener(HabboLoginDemoView.const_500,this.onInitConnection);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_58.mode = HabboConnectionType.const_92;
            if(this.var_173.keyExists("local.environment"))
            {
               this.var_58.mode = HabboConnectionType.const_1232;
            }
         }
         this.var_58.initConnection(HabboConnectionType.const_243);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_662);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2686 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_58.mode)
         {
            case HabboConnectionType.const_92:
               _loc6_ = "";
               _loc6_ = this.var_173.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_173.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1480 = _loc4_;
               this.var_233.loadBannerImage(_loc7_ + "?token=" + this.var_1480,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1232:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1678:
               this.var_993 = this.generateRandomHexString(30);
               this.var_540 = new DiffieHellman(new BigInteger(this.var_173.getKey("connection.development.prime"),16),new BigInteger(this.var_173.getKey("connection.development.generator"),16));
               this.var_540.init(this.var_993);
               _loc8_ = this.var_540.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_540.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_540.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2686);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1900,this.var_2684));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1749,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_654 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_654);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2685)
         {
            _loc2_ = this.var_173.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2685 = true;
         var _loc2_:String = this.var_173.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_173.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1480);
         ...
         this.var_540 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_540.init(_loc14_);
            _loc15_ = this.var_540.getPublicKey(10);
            if(_loc15_.length >= 64)
            {
               _loc12_ = _loc15_;
               this.var_993 = _loc14_;
            }
