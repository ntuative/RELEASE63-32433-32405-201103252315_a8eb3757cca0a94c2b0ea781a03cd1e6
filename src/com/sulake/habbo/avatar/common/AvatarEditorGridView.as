package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridView
   {
      ...
      public function AvatarEditorGridView(param1:IAvatarEditorCategoryModel, param2:String, param3:IHabboWindowManager, param4:IAssetLibrary)
      {
         ...
         if(this.var_17 != null)
         {
            this.var_587 = this.var_17.findChildByName("part_thumbs") as IItemGridWindow;
            this.var_485 = new Array();
            this.var_485.push(this.var_17.findChildByName("palette0") as IItemGridWindow);
            this.var_485.push(this.var_17.findChildByName("palette1") as IItemGridWindow);
         }
      }
      ...
      public function initFromList() : void
      {
         ...
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc1_:CategoryData = this.var_68.getCategoryData(this.var_1350);
         if(!_loc1_)
         {
            return;
         }
         this.var_587.removeGridItems();
         for each(_loc2_ in _loc1_.parts)
         {
            if(_loc2_)
            {
               this.var_587.addGridItem(_loc2_.view);
         ...
         while(_loc3_ < this.const_1363)
         {
            ...
            _loc5_ = this.var_485[_loc3_] as IItemGridWindow;
            if(!(!_loc4_ || !_loc5_))
            {
      ...
      {
         var _loc3_:IWindow = this.var_587.getGridItemAt(param1);
         if(!_loc3_)
         {
            return;
         }
      ...
      private function partEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            _loc3_ = this.var_587.getGridItemIndex(param1.window);
            this.var_68.selectPart(this.var_1350,_loc3_);
         }
      }
      ...
      private function paletteEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_DOWN)
         {
            ...
            {
               if(this.var_485.length > _loc7_)
               {
                  _loc4_ = this.var_485[_loc7_] as IItemGridWindow;
                  _loc5_ = _loc4_.getGridItemIndex(_loc6_);
                  if(_loc5_ > -1)
                  {
                     this.var_68.selectColor(this.var_1350,_loc5_,_loc7_);
                     return;
                  }
               }
               _loc7_++;
