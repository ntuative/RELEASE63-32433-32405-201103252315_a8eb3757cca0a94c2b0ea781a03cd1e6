package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_752:IFigurePartSet;
      ...
      private var var_610:Rectangle;
      
      private var var_1743:int = 0;
      
      private var var_430:IAvatarRenderManager;
      
      private var var_790:Boolean;
      
      private var var_1742:BitmapData;
      
      private const const_2049:int = 13421772;
      
      private const const_2050:int = 16777215;
      
      private const const_339:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1743 = Math.max(this.var_1743,_loc6_.colorLayerIndex);
            }
         }
         this.var_430 = this.var_68.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_689 != null && !this.var_1741)
            {
               _loc3_ = this.var_689;
            }
            else
         ...
         if(this.isSelected)
         {
            this.var_1410.color = this.const_2049;
         }
         else
         {
            this.var_1410.color = this.const_2050;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_430.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_430.isFigureReady(_loc1_))
         {
            this.var_430.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_339[_loc6_] + "_" + FigureData.const_135;
               _loc4_ = this.var_430.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_339.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_339[_loc6_] + "_" + FigureData.const_135;
                  _loc4_ = this.var_430.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_610 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1742)
               {
                  ...
                  this.var_1742 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_610.width,this.var_610.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_610.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_610.y;
